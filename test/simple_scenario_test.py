...
if __name__ == "__main__":
    ...
    server_simulator = SingleDeviceServerSimulator(
        device_starting_code, device_key, restricted_digit_set=restricted_digit_set
    )
    ...
    assert_time_equals(
        device_simulator.expiration_date, datetime.now() + timedelta(days=1)
    )

    print("\n")
    print(
        "Device: We enter the token a second time to make sure it doesnt add the "
        "days again"
    )
    device_simulator.enter_token(this_token)
    print("Device: We check the device status (should be active with 1 day)")
    ...
    assert device_simulator.count == server_simulator.count
    assert_time_equals(
        device_simulator.expiration_date, datetime.now() + timedelta(days=30)
    )
    ...
    assert device_simulator.count == server_simulator.count
    assert_time_equals(
        device_simulator.expiration_date, datetime.now() + timedelta(days=7)
    )
    ...
    print("\n")
    print(
        "Server: We generate a token for putting the device back PAYG-ON mode "
        "with 0 days"
    )
    this_token = server_simulator.generate_token_from_date(
    ...
    assert_time_equals(
        device_simulator.expiration_date, datetime.now() + timedelta(days=0)
    )
    ...
    device_simulator.enter_token(this_token)
    print(
        "Device: We check the device status (should be active with 1 day and "
        "the count synchronised with the server)"
    )
    device_simulator.print_status()
    assert device_simulator.count == server_simulator.count
    assert_time_equals(
    ...
    token_1 = server_simulator._generate_token_from_value(1, TokenType.ADD_TIME)
    token_2 = server_simulator._generate_token_from_value(1, TokenType.ADD_TIME)
    token_3 = server_simulator._generate_token_from_value(1, TokenType.ADD_TIME)
    token_4 = server_simulator._generate_token_from_value(1, TokenType.ADD_TIME)
    token_5 = server_simulator._generate_token_from_value(1, TokenType.ADD_TIME)
    token_6 = server_simulator._generate_token_from_value(1, TokenType.ADD_TIME)
    token_7 = server_simulator._generate_token_from_value(1, TokenType.ADD_TIME)
    token_8 = server_simulator._generate_token_from_value(1, TokenType.ADD_TIME)
    token_9 = server_simulator._generate_token_from_value(1, TokenType.ADD_TIME)
    print(
        "Tokens: ",
        token_1,
        token_2,
    ...
    print(
        "Device: We check the device status (should be active with +1 day (2 "
        "days total)"
    )
    device_simulator.print_status()
    assert device_simulator.count == server_simulator.count
    assert_time_equals(
        device_simulator.expiration_date, datetime.now() + timedelta(days=2)
    )
    print("Device: We enter the 1st token into the device")
    device_simulator.enter_token(token_1)
    print(
        "Device: We check the device status , it should not have changed, "
        "because its more than 5 add times before"
    )
    device_simulator.print_status()
    assert_time_equals(
        device_simulator.expiration_date, datetime.now() + timedelta(days=2)
    ...
    token_1 = server_simulator._generate_token_from_value(1, TokenType.ADD_TIME)
    token_2 = server_simulator._generate_token_from_value(0, TokenType.SET_TIME)
    ...
    print("\n")
    print("Server: We add generate 2 tokens, first add-time and then set-time")
    token_1 = server_simulator._generate_token_from_value(1, TokenType.SET_TIME)
    token_2 = server_simulator._generate_token_from_value(1, TokenType.ADD_TIME)
    print("Tokens: ", token_1, token_2)
    print("Device: We enter the 2nd token")
    device_simulator.enter_token(token_2)
